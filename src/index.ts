...
export interface JSONSchemaPrimitiveTypes {
  null: null;
  boolean: boolean;
  number: number;
  integer: number;
...
export interface ValueOnlyJSONSchema<T extends JSONSchemaPrimitive> {
...
export interface NumericJSONSchema {
  type: 'number' | 'integer';
  enum?: number[];
  const?: number;
  default?: number;
...
export interface TextJSONSchema<
  T extends JSONSchemaPrimitive = JSONSchemaPrimitive,
  F extends JSONSchemaFormat = JSONSchemaFormat,
  O extends Record<string, unknown> = Record<string, unknown>,
> {
  ...
  contentSchema?: JSONSchema<T, F, O>;
}
...
> {
  ...
  propertyNames?: TextJSONSchema<T, F, O> | ComposedJSONSchema<T, F, O>;
...
export interface ExpressiveTextJSONSchema<
  T extends JSONSchemaPrimitive = JSONSchemaPrimitive,
  F extends JSONSchemaFormat = JSONSchemaFormat,
  O extends Record<string, unknown> = Record<string, unknown>,
> {
  ...
  contentEncoding?: string;
  contentMediaType?: string;
  contentSchema?: ExpressiveJSONSchema<T, F, O>;
}

export interface ExpressiveArrayJSONSchema<
  T extends JSONSchemaPrimitive = JSONSchemaPrimitive,
  F extends JSONSchemaFormat = JSONSchemaFormat,
  O extends Record<string, unknown> = Record<string, unknown>,
> {
  ...
  default?: JsonArray;
  examples?: JsonArray[];
  items?: ExpressiveJSONSchema<T, F, O>;
  ...
  maxContains?: number;
  minContains?: number;
  unevaluatedItems?: ExpressiveJSONSchema<T, F, O>;
}

export interface ExpressiveObjectJSONSchema<
  T extends JSONSchemaPrimitive = JSONSchemaPrimitive,
  F extends JSONSchemaFormat = JSONSchemaFormat,
  O extends Record<string, unknown> = Record<string, unknown>,
> {
 ...
 * subtleties to understand the expected JSONs.
 *
 * Also allows to generate clean TypeScript types
...
> = BaseJSONSchema<T, F, O> &
  (
    ...
    | BooleanJSONSchema
    | NumericJSONSchema
    | ExpressiveTextJSONSchema<T, F, O>
    | ExpressiveArrayJSONSchema<T, F, O>
    | ExpressiveObjectJSONSchema<T, F, O>
    | {
        anyOf: [
          ExpressiveJSONSchema<T, F, O>,
          ...ExpressiveJSONSchema<T, F, O>[],
